    ...
    <div class="flex flex-wrap gap-3 items-end">
      ...
      <div class="flex-1 min-w-[180px]">
        ...
        <div class="relative group">
          ...
          <Search class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4 group-hover:text-orange-500 pointer-events-none" />
          <input :value="search" @input="$emit('update:search', $event.target.value)"
            placeholder="Busca por el nombre de la promoción..."
      ...
      <div v-if="!hideEstado" class="hidden sm:block w-px h-9 bg-gray-200 self-end mb-0.5"></div>
      ...
      <div>
        ...
        <select :value="tipopromocion" @change="$emit('update:tipopromocion', Number($event.target.value))"
          class="py-2.5 px-3 border-0 shadow-md bg-gray-50/80 rounded-2xl text-sm text-gray-700 outline-none focus:bg-white focus:ring-2 focus:ring-orange-500/20 cursor-pointer min-w-[140px]">
      ...
      <div>
        ...
        <select :value="idproducto" @change="$emit('update:idproducto', $event.target.value)"
          class="py-2.5 px-3 border-0 shadow-md bg-gray-50/80 rounded-2xl text-sm text-gray-700 outline-none focus:bg-white focus:ring-2 focus:ring-orange-500/20 cursor-pointer min-w-[140px]">
      ...
      <div>
        ...
        <div class="flex rounded-2xl bg-gray-100/80 p-1 gap-1">
          <button v-for="op in limiteOpciones" :key="op"
...
defineProps({
  ...
  tipopromocion: { type: Number, default: -1   },
  ...
  hideEstado:    { type: Boolean, default: false }
});

defineEmits(['update:search','update:estado','update:tipopromocion','update:idproducto','update:limite']);

const estadoOpciones = [
  { value: -1, label: 'Todas',     color: 'bg-gray-400'  },
  { value: 1,  label: 'Activas',   color: 'bg-green-500' },
  { value: 0,  label: 'Inactivas', color: 'bg-red-400'   },
];
