<template>
  <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 p-6 mt-8">
      ...
      <div class="flex-1 max-w-md">
        <div class="relative group">
          ...
          <div class="relative">
            ...
            <input
              :value="nombreFilter"
              @input="$emit('update:nombreFilter', $event.target.value)"
      ...
      <div class="flex flex-col">
        ...
        <div class="flex rounded-2xl bg-gray-100/80 p-1 gap-1">
          <button
            ...
            :key="status.value"
            @click="$emit('update:estadoFilter', status.value)"
            :class="[
              'px-4 py-2 text-sm font-medium rounded-xl transition-all duration-300',
              estadoFilter === status.value
...
const emit = defineEmits(['update:nombreFilter', 'update:estadoFilter', 'nuevoRepartidor']);
</script>
