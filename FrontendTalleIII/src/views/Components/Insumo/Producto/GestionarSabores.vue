<template>
        ...
        <div class="bg-gradient-to-r from-orange-500 to-red-600 p-6 text-white relative flex-shrink-0">
          <div class="flex items-center gap-4">
            <div class="p-3 bg-white/20 backdrop-blur-md rounded-2xl shadow-inner">
              <CakeSlice class="h-6 w-6 text-white" />
            </div>
            <div>
              <h2 class="text-2xl font-bold">Gestión de Sabores</h2>
              <p class="text-orange-100 text-sm font-medium">Administra los sabores para las variantes de producto</p>
            </div>
          </div>
          <button
            @click="$emit('cerrar')"
        ...
        <div class="overflow-y-auto flex-grow p-6 space-y-5">
          ...
          <div class="bg-orange-50/60 rounded-2xl p-4 space-y-3">
            <p class="text-sm font-semibold text-orange-700 flex items-center gap-1.5">
              ...
              {{ editando ? 'Editando sabor' : 'Nuevo sabor' }}
            </p>
            <div class="flex gap-2">
              <input
                ...
                @keyup.enter="guardar"
                :class="inputClass"
                :placeholder="editando ? 'Escribe el nuevo nombre...' : 'Ej: Chocolate, Vainilla, Queso...'"
                ref="nombreInput"
              />
              <button
                v-if="editando"
          ...
          <div v-else-if="sabores.length === 0" class="text-center py-10 text-gray-400">
            <CakeSlice class="h-10 w-10 text-gray-300 mx-auto mb-2" />
            <p class="text-sm">No se encontraron sabores</p>
          </div>
          ...
          <div v-else class="space-y-2">
            <div
              ...
              :class="['flex items-center justify-between px-4 py-3 bg-white rounded-2xl border shadow-sm transition-all',
                s.estado === 0 ? 'opacity-60 bg-gray-50 border-dashed' : 'border-gray-100',
                editandoId === s.idsabor ? 'border-orange-400 ring-2 ring-orange-200' : '']"
            >
              <div class="flex items-center gap-3">
                <div class="h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0"
                  :class="s.estado === 1 ? 'bg-orange-50 text-orange-500' : 'bg-gray-100 text-gray-400'">
                  <CakeSlice class="h-4.5 w-4.5" style="height:18px;width:18px" />
                </div>
                <div>
                  ...
                  <span :class="['text-[10px] px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider',
                    s.estado === 1 ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-600']">
              ...
              <div class="flex gap-1.5">
                ...
                <button
                  @click="confirmarToggle(s)"
                  :class="['w-7 h-7 border rounded-xl flex items-center justify-center transition-colors',
                    s.estado === 1 ? 'border-red-200 hover:bg-red-50 text-red-500' : 'border-green-200 hover:bg-green-50 text-green-500']"
                  :title="s.estado === 1 ? 'Desactivar' : 'Activar'"
                >
                  <component :is="s.estado === 1 ? EyeOff : Eye" class="h-3.5 w-3.5" />
                </button>
              </div>
            </div>
          </div>
  ...
  <Transition name="slide-up">
    <div v-if="notificacion.visible"
      class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gradient-to-r from-orange-500 to-red-600 text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-2 z-50 font-semibold">
...
import { X, Plus, Pencil, Save, LoaderCircle, Search, Eye, EyeOff, CheckCircle, AlertCircle, CakeSlice } from 'lucide-vue-next';
import { listarSabores, registrarSabor, updateSabor, toggleSabor } from '@/Server/Sabor';
...
const cargarSabores = async () => {
  ...
  try {
    ...
    const resp = await listarSabores(filtros.search || undefined, estadoParam, paginacion.paginaActual, filtros.limite);
    sabores.value = resp.data ?? [];
    paginacion.total = parseInt(resp.total) || 0;
    paginacion.totalPaginas = Math.ceil(paginacion.total / filtros.limite);
  } catch (err) {
    console.error(err);
    mostrarNotificacion('Error al cargar sabores');
  } finally {
...
const guardar = async () => {
  ...
  const duplicado = sabores.value.some(s =>
    s.idsabor !== editandoId.value && s.nombre.toLowerCase() === nombre.toLowerCase()
  );
  if (duplicado) { form.error = 'Ya existe un sabor con este nombre.'; return; }

  guardando.value = true;
  form.error = '';
  try {
    const resp = editando.value
      ? await updateSabor(editandoId.value, nombre)
      : await registrarSabor(nombre);
    mostrarNotificacion(resp?.message ?? (editando.value ? 'Sabor actualizado' : 'Sabor creado'));
    cancelarEdicion();
    await cargarSabores();
  } catch (err) {
    form.error = err.response?.data?.message ?? 'Error al guardar el sabor';
  } finally {
    guardando.value = false;
  }
};

const confirmarToggle = (s) => {
  modalConfirmacion.id = s.idsabor;
...
const onConfirmarToggle = async () => {
  ...
  if (!id) return;
  try {
    const resp = await toggleSabor(id);
