...
export const updateEntrega = async ({
    ...
    if (EntregaId) {
        const nuevoEntrega = await Entrega.findOne({
        ...
        if (!nuevoEntrega) {
          ...
          if (!nuevoEntrega?.Direccion?.Sucursal && IdSucursal==='TODOS') {
            if (tipoe !== "TPE-E1" && !DireccionId) {
                nuevoEntrega.Direccion = await createDireccion({
                    BarrioId: BarrioId || "",
                    Direccions: Direccions || "",
                    Referencia: Referencia || "",
                    Ubicacion: Ubicacion || ""
                ...
                await updateDireccion({ IdDireccion: DireccionId, BarrioId: BarrioId, Direccions: Direccions, Referencia: Referencia, Ubicacion: Ubicacion })

        } else {
            nuevoEntrega.Direccion = (tipoe === "TPE-E1") ?
                await cargarDireccion({ IdSucursal: IdSucursal }) :
                await updateDireccion({ IdDireccion: IdSucursal ? '' : DireccionId, BarrioId: BarrioId, Direccions:Direccions, Referencia: Referencia, Ubicacion: Ubicacion })
        }
        ...
        if (IdSucursal && tipoe === "TPE-E1") {
            ...
            if (direccion?.Sucursal) {
                ...
                await Direccion.delete({ IdDireccion: DireccionId });
            }
        }
        ...
        return createEntrega({ BarrioId: BarrioId, Direccions: Direccions, Referencia: Referencia, Ubicacion: Ubicacion, Costo: Costo, PedidoID: PedidoID, IdSucursal: IdSucursal, Fecha: Fecha, Hora: Hora, tipoe: tipoe })
};
