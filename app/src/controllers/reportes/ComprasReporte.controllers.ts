 ...
 */
export const getReporteComprasConsolidado = async (req: Request, res: Response) => {
    try {
        ...
        let filterIdx = 1;
        const params: any[] = [];
        const wheres: string[] = ["c.fechacompra BETWEEN $" + (filterIdx++) + " AND $" + (filterIdx++), "c.estado = 1"];
        ...
        const [rows, totalResult] = await Promise.all([
            Compra.query(sqlDetalle, [...params]),
            Compra.query(sqlTotal, [...params])
        ]);
 ...
 */
export const getReportePreciosHistorico = async (req: Request, res: Response) => {
    try {
        ...
        const data = await Compra.query(sqlPrecios, params);
