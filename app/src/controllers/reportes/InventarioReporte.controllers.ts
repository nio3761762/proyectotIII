 ...
 */
export const getReporteInventarioConsolidado = async (req: Request, res: Response) => {
    try {
        ...
        const data = await Inventario.query(sqlInventario, params);

        const procesado = data.map((item: any) => {
            ...
            const unidades = (item.unidades_disponibles || []).map((u: any) => {
        ...
        const grouped = procesado.reduce((acc: any, item: any) => {
        ...
        return res.json({
            ...
            resumen: {
                ...
                itemsAgotados: procesado.filter((i: any) => i.estado === 'AGOTADO').length,
                itemsStockBajo: procesado.filter((i: any) => i.estado === 'STOCK BAJO').length,
                valorTotalInventario: procesado.reduce((acc: number, curr: any) => acc + curr.valor_total, 0)
            }
        });
 ...
 */
export const getReporteKardex = async (req: Request, res: Response) => {
    try {
        ...
        const movimientos = await Inventario.query(sqlKardex, params);

        // Procesar y agrupar por tipo (INSUMO / PRODUCTO)
        const procesado = movimientos.reduce((acc: any, m: any) => {
            ...
            const unidades = (m.unidades_disponibles || []).map((u: any) => {
                const factor = Number(u.factor) || 1;
                const medida_cantidad = Number(u.medida_cantidad) || 1;
                const abreviatura = u.abreviatura || "";
            ...
            if (m.tipo_item === 'INSUMO') acc.insumos.push(itemProcesado);
            else acc.productos.push(itemProcesado);
