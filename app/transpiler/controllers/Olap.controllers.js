...
const getLatestMovements = async (req, res) => {
    try {
        ...
        const latestProductionsQuery = Produccion_1.Produccion.createQueryBuilder("produccion")
...
const calculateExistingStock = async (idSucursal, fechaFin) => {
    let query = db_1.AppDataSource.getRepository(Inventario_1.Inventario)
...
const getOlapData = async (req, res) => {
    try {
        ...
        const consultaPrincipal = db_1.AppDataSource.getRepository(DetalleVenta_1.Detalleventa)
            .createQueryBuilder("detalle")
...
const getRecentAlerts = async (req, res) => {
    try {
        ...
        const stockQuery = Inventario_1.Inventario.createQueryBuilder("inv")
            .leftJoinAndSelect("inv.Producto", "producto")
            .leftJoinAndSelect("inv.Sucursal", "sucursal");
        if (sucursalId) {
        ...
        const stockAlerts = stockRecords.filter(s => s.Producto && s.Cantidad <= (s.Producto.StockMinimo || 0));
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const sevenDaysFromNow = new Date(today);
        sevenDaysFromNow.setDate(today.getDate() + 7);
        const expiringPromotionsQuery = Promocion_1.Promocion.createQueryBuilder("promocion")
        ...
        const alerts = [
            ...pedidos.map(p => {
                ...
                const itemsRequested = p.Detallepedido.map(dp => {
                    if (dp.Producto)
                        return `${dp.Cantidad}x ${dp.Producto.Nombre}`;
                    if (dp.Productomedida)
                        return `${dp.Cantidad}x ${dp.Productomedida.Producto?.Nombre || 'Presentación'}`;
                    if (dp.Promocion)
                        return `${dp.Cantidad}x ${dp.Promocion.Nombre}`;
        ...
        const sortedAlerts = alerts.sort((a, b) => {
            ...
            while (timeA.length < 3)
                timeA.push(0);
            while (timeB.length < 3)
                timeB.push(0);
            const dateTimeA = new Date(dateA.getFullYear(), dateA.getMonth(), dateA.getDate(), timeA[0], timeA[1], timeA[2]);
            const dateTimeB = new Date(dateB.getFullYear(), dateB.getMonth(), dateB.getDate(), timeB[0], timeB[1], timeB[2]);
            return dateTimeB.getTime() - dateTimeA.getTime();
