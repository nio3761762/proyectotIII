...
exports.getReporteProduccionDetallado = exports.getReporteProduccionConsolidado = void 0;
const Produccion_1 = require("../../entities/Produccion");
/**
 * REPORTE CONSOLIDADO DE PRODUCCIÓN (Estricto por Receta)
 */
const getReporteProduccionConsolidado = async (req, res) => {
    try {
        ...
        let filterIdx = 1;
        const params = [];
        const wheres = ["p.fechaproduccion BETWEEN $" + (filterIdx++) + " AND $" + (filterIdx++), "p.estado = 1"];
        ...
        let costFilterIdx = 1;
        const costParams = [];
        const costWheres = ["p.fechaproduccion BETWEEN $" + (costFilterIdx++) + " AND $" + (costFilterIdx++), "p.estado = 1"];
    ...
    `;
        const [rows, costoResult] = await Promise.all([
            Produccion_1.Produccion.query(sql, params),
            Produccion_1.Produccion.query(sqlCosto, costParams)
        ]);
        ...
        for (const row of rows) {
            ...
            if (!suc.productos.has(prodKey)) {
                const ingredientesRaw = row.ingredientes || [];
        ...
        const sucursales = Array.from(sucursalMap.values()).map(s => ({
            ...
            productos: Array.from(s.productos.values()).map((prod) => {
                const cantProducida = prod.total_producto;
                const insumosCalculados = (prod.ingredientes || []).map((ing) => {
...
exports.getReporteProduccionConsolidado = getReporteProduccionConsolidado;
/**
 * REPORTE DETALLADO DE PRODUCCIÓN (Estricto por Receta con Empleado, Horno e Insumos Totales)
 */
const getReporteProduccionDetallado = async (req, res) => {
    try {
        const { fechadesde, fechahasta, idsucursal, idproducto } = req.query;
        if (!fechadesde || !fechahasta)
            return res.status(400).json({ message: "Fechas obligatorias." });
    ...
    `;
        ...
        const formatted = data.map((prod) => {
            ...
            const manoObra = (prod.ManoObraTotal || []).map((mo) => {
            ...
            const usoHornos = (prod.UsoHornosTotal || []).map((uh) => {
...
exports.getReporteProduccionDetallado = getReporteProduccionDetallado;
