...
exports.getReporteFinancieroConsolidado = void 0;
const Venta_1 = require("../../entities/Venta");
/**
 * REPORTE CONSOLIDADO FINANCIERO
 */
const getReporteFinancieroConsolidado = async (req, res) => {
    try {
        ...
        const [sucRaw, ingRaw, revRaw, gastRaw, salRaw, compRaw, vDiariasRaw, cDiariasRaw, revDiariosRaw, gDiariosRaw, ggRaw, ggDetRaw, hoyRaw] = await Promise.all([
            Venta_1.Venta.query(sqlSucursales, [sucursalId]),
            Venta_1.Venta.query(sqlIngresosTienda, params),
            Venta_1.Venta.query(sqlRevendedores, params),
            Venta_1.Venta.query(sqlGastos, params),
            Venta_1.Venta.query(sqlSalarios, params),
            Venta_1.Venta.query(sqlCompras, params),
            Venta_1.Venta.query(sqlVentasDiarias, params),
            Venta_1.Venta.query(sqlComprasDiarias, params),
            Venta_1.Venta.query(sqlIngresosRevendedoresDiarios, params),
            Venta_1.Venta.query(sqlGastosDiarios, params),
            Venta_1.Venta.query(sqlGastosGenerales, [fechadesde, fechahasta]),
            Venta_1.Venta.query(sqlGastosGeneralesDetalle, [fechadesde, fechahasta]),
            Venta_1.Venta.query(sqlHoy, [sucursalId])
        ]);
        ...
        const resumenHoy = hoyRaw.map((dataHoy) => {
        ...
        const reporteSucursales = sucRaw.map((s) => {
            const ing = ingRaw.find((r) => r.idsucursal === s.idsucursal)?.total || 0;
            const rev = revRaw.find((r) => r.idsucursal === s.idsucursal) || { venta_bruta: 0, comisiones: 0 };
            const gast = gastRaw.find((r) => r.idsucursal === s.idsucursal) || { total: 0, detalle: [] };
            const sal = salRaw.find((r) => r.idsucursal === s.idsucursal) || { total: 0, planilla: [] };
        ...
        const listaFechas = [];
        let fechaActualLoop = new Date(fechadesde);
        const fechaFinLoop = new Date(fechahasta);
        ...
        // Resumen General
        const totalGastosOp = reporteSucursales.reduce((acc, curr) => acc + curr.egresos.totalGastos, 0);
        const gananciaOp = reporteSucursales.reduce((acc, curr) => acc + curr.utilidadOperativa, 0);
        const resumenGeneral = {
            totalIngresos: reporteSucursales.reduce((acc, curr) => acc + curr.ingresos.totalGananciaBakery, 0),
...
exports.getReporteFinancieroConsolidado = getReporteFinancieroConsolidado;
